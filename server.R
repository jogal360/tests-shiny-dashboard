...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$go, {
    output$dash <- renderUI({
      ...
      shinyjs::hide("ocultar", anim = FALSE, animType = "fade")
      shinyjs::show("idheader", anim = TRUE, animType = "fade")
      shinyjs::removeClass("idBody", "sidebar-collapse")
      shinyjs::show("idsidebar", anim = TRUE, animType = "fade")
      
      #Secciones del dashboard body
      tabItems(
        #Dashboard body de inicio - default
        tabItem(
          tabName = "hm",
          class = "active text-center",
          tags$h1("Instrucciones"),
          tags$p("Para comenzar, es necesario ingresar datos al sistema."),
          p(
            "Para esto, presiona en 'Ingresar datos' en el panel de la izquierda."
          ),
          p(
            "Luego elige un método de ingresar los datos y presiona el botón 'Seleccionar'"
          )
        ),
        #Body al elegir el metodo de ingreso de datos
        tabItem(
          tabName = "loadData",
  ...
  observeEvent(input$btnCalcR, {
    output$resuReg <- renderUI({
      fluidRow(
        column(class="text-center",
               12, tags$br(),h3(paste("Regresión de", y, " sobre ", x))
        ),
